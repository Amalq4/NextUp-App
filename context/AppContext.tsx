...
import { UserProfile, ListEntry, ProgressEntry, Friend, ListStatus, MediaType } from '@/types/media';
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateListStatus = useCallback(async (mediaId: number, status: ListStatus) => {
    setState(prev => {
  ...
  const getListByStatus = useCallback((status: ListStatus) => {
    return state.lists.filter(e => e.status === status);
  }, [state.lists]);

  const getListEntry = useCallback((mediaId: number) => {
  ...
  const value = useMemo(() => ({
    ...
    removeFriend,
    clearAllData,
  }), [state, saveProfile, addToList, removeFromList, updateListStatus, getListByStatus, getListEntry, updateProgress, getProgress, addFriend, removeFriend, clearAllData]);
